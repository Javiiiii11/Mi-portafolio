...
const cursorType = ref<"circle-black" | "arrow" | "arrow-external" | "circle-white" | "search" | null>(null);
const detectedType = ref<"circle-black" | "arrow" | "arrow-external" | "circle-white" | "search" | null>(null);
...
): "circle-black" | "arrow" | "arrow-external" | "circle-white" | "search" | null => {
  if (!element) return null;
  if (element instanceof HTMLElement) {
    const cursor = element.dataset.cursor;
    if (cursor === "circle-black" || cursor === "arrow" || cursor === "arrow-external" || cursor === "circle-white" || cursor === "search") {
      return cursor;
    }
  }
...
<template>
  <div ref="cursorWrapperRef" class="cursor-wrapper" :class="{ [`project-${projectId}`]: projectId !== null }">
    <div ref="cursorScaleRef" class="cursor-scale">
      ...
      <div class="cursor cursor-search" :class="{ 'cursor-active': cursorType === 'search' }">
        <Search class="cursor-search-icon" />
      </div>
    </div>
  </div>
</template>
